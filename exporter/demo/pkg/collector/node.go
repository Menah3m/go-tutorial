...
type nodeStatsMetrics []struct {
	...
	cpuval  func(*cpu.InfoStat) float64
...
func NewNodeCollector() prometheus.Collector {
	...
	return &NodeCollector{
		requestDesc: prometheus.NewDesc(
			...
			prometheus.Labels{"STATIC_LABEL1": "静态值可以放在这里", "HOST_NAME": hostName, "HOST_UPTIME": string(hostUptime)}),
		nodeMetrics: nodeStatsMetrics{
			{
				...
				eval:    func(ms *mem.VirtualMemoryStat) float64 { return float64(ms.Total) / (1024 * 1024 * 1024) },
			},
			{
				desc: prometheus.NewDesc(
					"free_mem",
					...
					nil, nil),
				valType: prometheus.GaugeValue,
				eval:    func(ms *mem.VirtualMemoryStat) float64 { return float64(ms.Free) / (1024 * 1024 * 1024) },
			},
		},
